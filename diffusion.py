...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, model, x_start, con_emb, steps, sampling_noise=False):
        ...
        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(model, x_t, con_emb, t)
            if sampling_noise:
    ...
    def training_losses(self, model, x_start, con_emb, reweight=False):
        batch_size, device = x_start.size(0), x_start.device
        ts, pt = self.sample_timesteps(batch_size, 'importance')
        ...
        # print(x_t.shape)
        # print(con_emb.shape)
        model_output = model(x_t, con_emb, ts)
